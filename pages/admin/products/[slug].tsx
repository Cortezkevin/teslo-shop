...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onAddTag = (  { code }: KeyboardEvent<HTMLDivElement> ) => {
    if( code === 'Space') {
      setValue(
        'tags', 
        [ ...getValues('tags'), newTagValue.trim().toLowerCase()], 
        { shouldValidate: true }
      );
      setNewTagValue('');
    }
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              { ...register('slug', {
                required: 'This field is required',
                minLength: { value: 2, message: 'Must be 2 characters' },
                validate: (val) => val.trim().includes(' ') || val.trim().includes('-') ? 'Cannot have blank spaces or scripts (-)' : undefined
              })}
              error={ !!errors.slug }
